#' @title Fit CIFs using while on treatment strategy for competing risks data, based on efficient influence functions
#'
...
surv.whileon.eff <- function(A,Time,cstatus,X=NULL){
  n = length(A)
  if (is.null(X)){
  ...
  cif1 = t(apply(exp(-cumhaz1)*lam11,1,cumsum))
  cif0 = t(apply(exp(-cumhaz0)*lam10,1,cumsum))
  cif1x = A*ips*t(apply(exp(-cumhaz1)*dMP11,1,cumsum))-
    A*ips*cif1*t(apply(dMP11+dMP21,1,cumsum))+
    A*ips*t(apply(cif1*(dMP11+dMP21),1,cumsum))+cif1
