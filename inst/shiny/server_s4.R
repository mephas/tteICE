...
if (input$tbd_removed){
  ...
  if(!input$scr) 
    fit1 <- surv.tteICE(A=A_32(), Time=TIME_32(), cstatus=CSTATUS_32(), strategy='removed', cov1=COV2(), method = input$meth, weights = WEIGHT()) 
  else
    fit1 <- scr.tteICE(A=A_32(), Time=TIME_32(), status=CSTATUS_32(), Time_int=TIME_321(), status_int=CSTATUS_321(), strategy='removed', cov1=COV2(), method = input$meth, weights = WEIGHT()) 
} else {fit1 <- NULL}
...
hsr_32_plot1 <- eventReactive(input$B_32_surv,{
  if(length(hsr_32())!=0) plot(hsr_32(), type="ate", decrease = as.logical(input$d_320), conf.int = input$conf, nboot = input$bs_320, seed = 0, ylim=input$yrange)
})
hsr_32_plot2 <- eventReactive(input$B_32_surv,{
  if(length(hsr_32())!=0) {
    plot(hsr_32(), type="inc", decrease = as.logical(input$d_320), conf.int = input$conf, nboot = input$bs_320, seed = 0, ylim=input$yrangecif, 
