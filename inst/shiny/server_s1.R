...
if (input$tbd_treatment){
  if(!input$scr)
    fit1 <- surv.tteICE(A=A_32(), Time=TIME_32(), cstatus=CSTATUS_32(), strategy='treatment', cov1=COV2(), method = input$meth, weights = WEIGHT())
  else
    fit1 <- scr.tteICE(A=A_32(), Time=TIME_32(), status=CSTATUS_32(), Time_int=TIME_321(), status_int=CSTATUS_321(), strategy='treatment', cov1=COV2(), method = input$meth, weights = WEIGHT())
...
tps_32_plot1 <- eventReactive(input$B_32_surv,{
  if(length(tps_32())!=0) plot(tps_32(), type="ate", decrease = as.logical(input$d_320), conf.int = input$conf, nboot = input$bs_320, seed = 0, ylim=input$yrange)
})
tps_32_plot2 <- eventReactive(input$B_32_surv,{
  if(length(tps_32())!=0) {
    # if(input$adp) adp=TRUE else adp=FALSE
    plot(tps_32(), type="inc", decrease = as.logical(input$d_320), conf.int = input$conf, nboot = input$bs_320, seed = 0, ylim=input$yrangecif, 
