...
hsn_32_plot2 <- eventReactive(input$B_32_surv,{
  if(length(hsn_32())!=0) {
    plot(hsn_32(), type="inc", decrease = input$d_320, conf.int = input$conf, nboot = input$bs_320, seed = 0, ylim=input$yrangecif, legend.inc=c(input$t1, input$t0))
    if(input$adp) {
      p = hsn_32()$p.val
      if(is.null(p)) p=NA
...
## Prediction ---------------------------
hsnbs_32_tab_pred <- reactive({
if((length(hsn_32())!=0) & (length(input$bstime_320)!=0)){

fit <- hsn_32()
time.point <- as.numeric(input$num3)
