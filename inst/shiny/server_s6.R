...
pss_32_plot1 <- eventReactive(input$B_32_surv,{
  if(length(pss_32())!=0) plot(pss_32(), type="ate", decrease = as.logical(input$d_320), conf.int = input$conf, ylim=input$yrange, xlab=input$xlab)
})
pss_32_plot2 <- eventReactive(input$B_32_surv,{
  if(length(pss_32())!=0) {
    plot(pss_32(), type="inc", decrease = as.logical(input$d_320), conf.int = input$conf, ylim=input$yrangecif, xlab=input$xlab, 
